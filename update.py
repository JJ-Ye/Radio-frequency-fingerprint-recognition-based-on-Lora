...
class LocalUpdate(object):
    def __init__(self, args, dataset, idxs, logger):
        ...
        #self.loss_func = CombinedLoss(margin=1.0, alpha=0.5)
    ...
    def train_val_test(self, dataset, idxs):
        ...
        #print(np.array(idxs_train).shape,np.array(idxs_val).shape,np.array(idxs_test).shape)
            ...
            for batch_idx, (images, labels) in enumerate(self.train_loader):
                ...
                if self.args.verbose and (batch_idx % 100 == 0):
                    print('| Global Round : {} | Local Epoch : {} | [{}/{} ({:.0f}%)]\tLoss: {:.6f}'.format(
                        global_round + 1, idx, batch_idx * len(images),
...
def test_inference_stft(args, model, test_dataset,test_y):
    """ Returns the test accuracy and loss.
    """
    ...
    loss_func = nn.CrossEntropyLoss().to(device)
    idx = [i for i in range(len(test_dataset))]
    test_loader = DataLoader(DatasetSplit_stft(test_dataset,test_y, idx), batch_size=128,
                             shuffle=True)
    predictions, truths = [], []
