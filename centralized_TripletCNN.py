...
from models import MLP,  CNN,CNN2D,RFSignalCNN,ResNet18
...
if __name__ == '__main__':
    ...
    margin = 0.1
    alpha = 0.5
    loss_func = CombinedLoss(margin, alpha)
    print("***使用组合损失函数，***")
    print(f"margin={margin}, alpha={alpha}")
    ...
    for epoch in range(args.epochs):
        ...
        for batch_idx, (data, labels) in enumerate(trainloader):
            ...
            # 初始化正负样本 PyTorch 张量
            positive_samples = torch.empty((data.size(0), *data.shape[1:]), dtype=torch.float32)
            negative_samples = torch.empty((data.size(0), *data.shape[1:]), dtype=torch.float32)

            # 遍历每个样本
            for i in range(data.size(0)):
                sample_data = data[i].unsqueeze(0)  # 获取当前样本
                sample_label = labels[i]  # 获取当前样本的标签

                # 从同一类中随机选择正样本
                same_label_indices = (labels == sample_label).nonzero(as_tuple=True)[0]
                same_label_indices = same_label_indices[same_label_indices != i]  # 排除当前样本
                if len(same_label_indices) > 0:
                    positive_idx = random.choice(same_label_indices.tolist())
                    positive_samples[i] = data[positive_idx]  # 存入正样本
                else:
                    # 如果同类样本没有，选择当前样本作为正样本
                    positive_samples[i] = sample_data

                # 从不同类中随机选择负样本
                different_label_indices = (labels != sample_label).nonzero(as_tuple=True)[0]
                negative_idx = random.choice(different_label_indices.tolist())
                negative_samples[i] = data[negative_idx]  # 直接存入张量

            labels = labels.to(device)

            optimizer.zero_grad()
            anchor_features, outputs = global_model(data)
            positive_features, _ = global_model(positive_samples)
            negative_features, _ = global_model(negative_samples)

            loss = loss_func(anchor_features, positive_features, negative_features, outputs, labels)
            loss.backward()
            optimizer.step()
            # 累计损失
            train_loss_sum += loss.item() * data.size(0)    #获取当前batch批次的损失值。loss 是一个张量，loss.item() 将其转化为一个普通 Python 数字。
    ...
    if (args.process == 'RAW_IQ'):
        tra_acc, tra_loss, tra_pred, tra_gt = test_inference(args, global_model, train_dataset)
    print(f"Test Accuracy on Train: {tra_acc:.2%}")
    if (args.process == 'stft'):
    ...
    print(f"Test Accuracy on Test: {te_acc:.2%}")

    # 保存完整模型（结构+权重）
    model_path = '../save/TripletCNN/centralized_Tripletmodel_{:}_{:}_{:}.pth'.format(args.signal, args.model, args.snr_high)
    torch.save(global_model, model_path)

    # 保存权重（仅参数）
    weights_path = '../save/TripletCNN/centralized_Tripletweights_{:}_{:}_{:}.pt'.format(args.signal, args.model, args.snr_high)
    torch.save(global_model.state_dict(), weights_path)

    # 保存日志文件（HDF5）
    h5_path = '../save/TripletCNN/centralized_log_{:}_{:}_{:}.h5'.format(args.signal, args.model, args.snr_high)
